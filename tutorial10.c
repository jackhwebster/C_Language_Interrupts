    /*Tutorial 10 Code written by Jack Webster adapted from ELEC 371 Template
    provided by N. Manjikian. 
    Switches 0-5 control the first 6 LEDS, while Switches 6-9 toggle LED hex
    display at an interval of 0.5s (controlled by timer interrupts).
    Main program checks for character input and echoes if a character is 
    available*/
...
unsigned int getSwitches(void);
void toggleHEX(void);
void updateSixLEDS(unsigned int pattern);


//global variables
unsigned int hex_display_state; //needs a global variable since it can be changed in more than one way (interrupt + main program)
...
void Init (void)
{
    ...
    *TIMER_CONTROL = 7;
    ...
    NIOS2_WRITE_IENABLE(0x1); //enable timer interrupts

    NIOS2_WRITE_STATUS(0x1); //IE bit to 1 to allow processor interrupts
}
...
void toggleHEX(){
    unsigned int switchPattern = getSwitches();
    unsigned int hexPattern;
    unsigned int bits = *HEX_DISPLAY;
    unsigned int toggledBits = bits ^ 0xFFFFFFFF; //toggle entire sequence, then we can keep what's relevant to the switches
    if ((switchPattern & 0x40) == 0x40){ //switch 7
        hexPattern = toggledBits & 0xFF;
        bits = bits & 0xFFFFFF00; 
        bits = bits | hexPattern; //put new pattern into rightmost display
    }
    if ((switchPattern & 0x80) == 0x80){ //switch 8
        hexPattern = toggledBits & 0xFF00;
        bits = bits & 0xFFFF00FF; 
        bits = bits | hexPattern; //put new pattern into rightmost display
    }
    if ((switchPattern & 0x100) == 0x100){ //switch 9
        hexPattern = toggledBits & 0xFF0000;
        bits = bits & 0xFF00FFFF; 
        bits = bits | hexPattern; //put new pattern into rightmost display
    }
    if ((switchPattern & 0x200) == 0x200){ //switch 10
        hexPattern = toggledBits & 0xFF000000;
...
unsigned int getSwitches(){ //divide into what it does for first 4 and bottom 4 switches
    return (*SWITCHES & 0x3FF); //we are only dealing with the first 10 switches
}

void mainEcho(){
    unsigned int ch = checkChar();
    if (ch != 0){
        printChar(ch);
    }
}
...
void mainLEDS(){
    unsigned int pattern = getSwitches();
